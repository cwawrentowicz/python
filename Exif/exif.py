...
def thumb_nail(i, folder,entry):
    ...
    try:
        ...
        print("Błąd 3 przy tworzeniu miniatury")
        return 1
...
def main():
    ...
    sygnatura = EntrySygnatura.get() 
    oznaczenie = EntryOznaczenie.get()
    osoba = EntryOsoba.get()
    print("Wczytuję listę plików ......")
    #files_list = getListOfFilesPic("C:\\Users\\c.wawrentowicz\\pictures\\scan")
    files_list = getListOfFilesPic(txtFolder.get())
...
if __name__ == '__main__':
    ...
    def clicked():
        main()             
        lbl1.configure(text="Wykonane !!")
  
    root = tkinter.Tk()
    root.geometry("500x220")
    root.title("Exif")
    folder_path = tkinter.StringVar()
    txtFolder = tkinter.Entry(root,width=45,textvariable=folder_path)
    txtFolder.place(x=10, y=10)
    button1 = tkinter.Button(text="Wybierz folder", command=browse_button)
    button1.place(x=310, y=10)

    lblSygnatura = tkinter.Label(root, text="Sygnatura")
    lblSygnatura.place(x=10, y=40)
    EntrySygnatura = tkinter.Entry(root,width=25)
    EntrySygnatura.place(x=100, y=40)

    lblOznaczenie = tkinter.Label(root, text="Oznaczenie")
    lblOznaczenie.place(x=10, y=70)
    EntryOznaczenie = tkinter.Entry(root,width=35)
    EntryOznaczenie.place(x=100, y=70)

    lblOsoba = tkinter.Label(root, text="Osoba")
    lblOsoba.place(x=10, y=100)
    EntryOsoba = tkinter.Entry(root,width=35)
    EntryOsoba.place(x=100, y=100)

    start = tkinter.Button(root, text="Start", command=clicked)
    start.place(x=10, y=130)

                      
    lbl1 = tkinter.Label(root, text=".")
    lbl1.place(x=10, y=160)
